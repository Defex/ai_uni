...
class NeuralNetwork50():
    def __init__(self):
        ...
        self.syn0 = 0.5*np.random.random((50*2,500)) - 0.25
    ...
    def load_test_match(self, training_data):
        if(self.firstTestInput):
            self.Xtest = np.array([training_data[0:50*2:1]])
            self.Ytest = np.array([[training_data[50*2]]])
            self.results = self.Ytest
            self.firstTestInput = False
        else:
            self.Xtest = np.append(self.Xtest, [training_data[0:50*2:1]], axis=0)
            self.Ytest = np.append(self.Ytest, [[training_data[50*2]]], axis=0)
            self.results = self.Ytest
    ...
    def load_one_match(self, training_data):
        if(self.firstInput):
            self.X = np.array([training_data[0:50*2:1]])
            self.Y = np.array([[training_data[50*2]]])
            self.firstInput = False
        else:
            self.X = np.append(self.X, [training_data[0:50*2:1]], axis=0)
            self.Y = np.append(self.Y, [[training_data[50*2]]], axis=0)
    ...
    def train_times(self, n):
        for j in range(n):
            ...
            if (j%1 == 0):
                ...
                print("50 popular champs mse: " + str("{0:.5f}".format(mse[0])))
